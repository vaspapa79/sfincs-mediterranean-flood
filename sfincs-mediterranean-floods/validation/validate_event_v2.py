...
def _validate_ems_v2(cfg, s):
    ...
    # ----- 3. Asymmetric fuzzy: dilate observed only (Wing et al. 2017) -----
    # Counts a model wet cell as TP if it lies within r cells of an
    # observed wet cell. This is the operational early-warning convention.
    fuzzy_obs_only = {}
    for r in (0, 1, 2, 3):
        ems_d = _dilate(ems_wet, r)
        # Also widen the model wet so an observed cell within r of a model
        # cell counts as TP (symmetric in the operational sense).
        model_d = _dilate(model_wet, r)
        c = _contingency_full(model_d, ems_d, evaluation_mask)
        fuzzy_obs_only[f"r={r}"] = c
    ...
    return {
        ...
        "fuzzy_asym_at_h_gt_0p10m": fuzzy_obs_only,
